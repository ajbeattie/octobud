...
const DB_VERSION = 4; // Stores: pollState, auth, queryConfig
...
function openDB() {
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            const requiredStores = ['pollState', 'auth', 'queryConfig'];
                ...
                try {
                    ...
                    const reopenedDb = await new Promise((reopenResolve, reopenReject) => {
                        ...
                        reopenRequest.onupgradeneeded = (event) => {
                            ...
                            if (!newDb.objectStoreNames.contains('auth')) {
                                newDb.createObjectStore('auth', { keyPath: 'id' });
                            }
        ...
        request.onupgradeneeded = (event) => {
            ...
            // Store for auth token (for service worker to access JWT)
...
async function getAuthToken() {
    try {
        ...
        // Check if object store exists
        if (!db.objectStoreNames.contains('auth')) {
            console.warn('[SW] auth object store missing, returning null token');
            return null;
        }
...
// Notify clients that token has expired
async function notifyTokenExpired() {
    try {
        console.warn('[SW] Token expired, requesting refresh from main app');
        const clients = await self.clients.matchAll({ includeUncontrolled: true, type: 'window' });
        clients.forEach(client => {
            client.postMessage({ type: 'TOKEN_EXPIRED' });
        });
    } catch (error) {
        console.error('[SW] Failed to notify clients of token expiration:', error);
    }
}
...
async function fetchNotifications() {
    try {
        ...
        if (response.status === 401) {
            debugLog('[SW] Token expired, notifying clients');
