<script lang="ts">
	...
	onMount(() => {
		...
		if (browser) {
			...
			(async () => {
				...
				const isSetupPage = window.location.pathname === "/initial-setup";
				...
				if (!token && !isLoginPage && !isSetupPage) {
					...
					try {
						...
						if (isLoginPage) {
							// Already logged in, check sync settings
							if (hasConfigured) {
								void goto(resolve("/views/inbox" as any));
							} else {
								void goto(resolve("/initial-setup" as any));
							}
							return;
						} else if (!isSetupPage && !hasConfigured) {
							// Not on setup page but sync settings not configured
							void goto(resolve("/initial-setup" as any));
							return;
						} else if (isSetupPage && hasConfigured) {
						...
						// If we can't check, allow access but log error
					}
				}

				// Only initialize app features if not on login or setup page
		...
		// Initialize theme store (only if not on login or setup page)
		if (
			browser &&
			window.location.pathname !== "/login" &&
			window.location.pathname !== "/initial-setup"
		) {
			getThemeStore();
		}

		// Register service worker for background sync (only if not on login or setup page)
		...
		// Periodic check to ensure SW polling is active when authenticated
		// This acts as a catchall for various routing scenarios (e.g., after login redirect)
		...
		swPollingCheckInterval = setInterval(async () => {
			// Only check if authenticated and not on login/setup pages
			const token = getAuthToken();
			const isLoginPage = window.location.pathname === "/login";
			const isSetupPage = window.location.pathname === "/initial-setup";
	...
	afterNavigate(({ from, to, type }) => {
		...
		// When navigating from login/setup to an authenticated route, ensure SW polling is active
		// This handles the case where SW wasn't started during login flow
		const fromLogin = from?.url?.pathname === "/login";
		const fromSetup = from?.url?.pathname === "/initial-setup";
		const toProtectedRoute =
			to?.url?.pathname !== "/login" && to?.url?.pathname !== "/initial-setup";

		if ((fromLogin || fromSetup) && toProtectedRoute && getAuthToken()) {
	...
	$: isSetupRoute = $pageStore.url.pathname === "/initial-setup";
