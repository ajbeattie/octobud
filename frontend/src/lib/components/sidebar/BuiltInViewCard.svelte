<script lang="ts">
	...
	$: iconStrokeWidth = icon === "infinity" ? undefined : "1.5";
</script>

<li>
	<button
		type="button"
		class={`group relative flex w-full items-center gap-2 rounded-md border border-transparent px-2 py-1.5 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/40 ${
			selected
		...
		<span class="flex flex-1 flex-col min-w-0">
			<span class="flex items-center justify-between gap-2">
				...
				{#if count !== undefined && count > 0}
					<span
						class={`flex h-6 flex-shrink-0 items-center justify-center rounded-full px-2 py-1 text-xs font-regular transition-opacity delay-200 ${
							selected
