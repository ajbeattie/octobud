...
<div
	class="absolute top-0 z-[100] flex h-screen w-[15rem] xl:w-[16rem] 2xl:w-[18rem] flex-col border-r border-gray-200 bg-gray-100 shadow-2xl dark:border-gray-800 dark:bg-gray-950"
	role="navigation"
...
>
	<div
		class="flex flex-1 flex-col gap-2 overflow-y-auto pl-2 pr-2 pb-32 pt-4"
			...
			<ul class="space-y-0 py-2">
				{#each builtInViewList as builtIn (builtIn.id)}
					<BuiltInViewCard
						...
						icon={builtIn.icon as "inbox" | "infinity" | "check" | "archive" | "snooze"}
			...
			{#if !viewsCollapsed}
				...
				<div class="mt-1 pl-1 flex items-center justify-between">
					{#if userViews.length > 1}
						<button
							type="button"
							class="flex-shrink-0 whitespace-nowrap rounded-full px-1 py-1 text-xs font-semibold text-gray-500 dark:text-gray-500 transition hover:text-gray-800 dark:hover:text-gray-300"
					...
					<button
						type="button"
						class="mr-2 flex-shrink-0 whitespace-nowrap rounded-full px-1 py-1 text-xs font-semibold text-gray-500 dark:text-gray-500 transition hover:text-gray-800 dark:hover:text-gray-300"
						on:click={onNewView}
					>
						Add
					</button>
