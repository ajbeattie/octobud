<script lang="ts">
	...
	export let onSelect: (tagId: string) => void | Promise<void> = () => {};
	export let onEdit: (tag: Tag) => void = () => {};
	export let onDragStart: (tagId: string) => void = () => {};
	export let onDragOver: (e: DragEvent, tagId: string) => void = () => {};
	export let onDrop: (e: DragEvent, tagId: string) => void = () => {};
...
>
	...
	>
		...
		<div
			...
			class="flex-1 px-2 py-1.5 text-left outline-none focus-visible:ring-2 focus-visible:ring-blue-600/40"
		...
		>
			<div class="flex items-center gap-2 pr-10">
				<span class="flex flex-shrink-0 items-center justify-center" aria-hidden="true">
					<svg
						class="h-[18px] w-[18px]"
						viewBox="0 0 24 24"
						fill="currentColor"
					...
					<div class="flex items-center gap-1.5">
						<span
							class={`text-[14px] font-normal truncate ${
								selected ? "text-gray-900 dark:text-gray-100" : "text-gray-900 dark:text-gray-100"
							}`}
						>
							{tag.name}
						</span>
		...
		{#if tag.unreadCount !== undefined && tag.unreadCount > 0 && !(showActions && hovered) && !reorderMode}
			<span
				class={`absolute right-2 top-1/2 flex h-6 -translate-y-1/2 items-center justify-center rounded-full px-2 text-[12px] font-regular transition-opacity delay-200 ${
					selected
						? "bg-gray-200 text-gray-900 dark:bg-gray-700 dark:text-gray-100"
						: "bg-gray-100/60 text-gray-700 dark:bg-gray-800/60 dark:text-gray-100"
				}`}
				aria-label={`${tag.unreadCount} new notification${tag.unreadCount === 1 ? "" : "s"}`}
			>
				{tag.unreadCount}
			</span>
		{/if}
		{#if showActions && (hovered || actionsOpen) && !reorderMode}
			<button
				type="button"
				class="absolute right-2 top-1/2 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-transparent bg-gray-200/80 text-gray-600 transition hover:bg-gray-300/80 hover:text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/40 dark:bg-gray-900/80 dark:text-gray-400 dark:hover:bg-gray-800/80 dark:hover:text-gray-300"
			...
			{#if actionsOpen}
				...
				>
					<button
						type="button"
						class="flex w-full items-center justify-start gap-2 px-3 py-2 text-left text-gray-700 transition hover:bg-gray-50 focus-visible:outline-none focus-visible:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-900/50 dark:focus-visible:bg-gray-900/50"
