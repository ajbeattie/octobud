<script lang="ts">
	...
	let singleModeComponent: any = null;
	let splitModeComponent: any = null;
	...
	export function saveScrollPosition() {
		const listSection = splitModeEnabled
			? splitModeComponent?.listSectionComponent
			: singleModeComponent?.listSectionComponent;
...
{:else}
	<section class="flex min-h-0 flex-1 flex-col gap-5 overflow-hidden">
		...
		{#if splitModeEnabled}
			<NotificationSplitMode
				bind:this={splitModeComponent}
				{hasActiveFilters}
				{totalCount}
				{pageRangeStart}
				{pageRangeEnd}
