	...
	{#if commands.length > 0}
		{#each commands as command, index (command.id)}
			<button
				...
				class={`flex w-full items-center justify-between gap-4 rounded-md px-3 py-2 text-left text-sm text-gray-900 dark:text-gray-200 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600/60 ${
					index === selectedIndex
