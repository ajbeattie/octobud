	...
	<form class="space-y-5" on:submit|preventDefault={submit}>
		<div class="rounded-xl bg-gray-50 dark:bg-gray-900/80 p-5 space-y-5">
			<div class="space-y-2">
				...
				<div class="flex gap-3">
					...
					<input
						id="filter-name"
						class="flex-1 rounded-lg border px-3 py-2 text-sm text-gray-900 dark:text-gray-200 outline-none transition focus:ring-2 focus:ring-blue-600/30 ${error
							? 'border-rose-500 focus:border-rose-500'
							: 'border-gray-300 dark:border-gray-800 focus:border-blue-600'} bg-white dark:bg-gray-950"
			...
			<div class="space-y-2">
				...
				<input
					...
					class="w-full rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 px-3 py-2 text-sm text-gray-900 dark:text-gray-200 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-600/30"
			...
			<div class="space-y-2">
				<div class="flex items-center justify-between gap-3">
					...
					<button
						type="button"
						class="text-xs text-blue-500 transition hover:text-blue-400 flex-shrink-0"
				...
				<div class="space-y-3 pt-2 border-t border-gray-200 dark:border-gray-800">
					<label class="flex items-center gap-3 cursor-pointer">
						<input
							...
							class="w-4 h-4 rounded border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-indigo-600 focus:ring-2 focus:ring-indigo-600/30 focus:ring-offset-0"
						/>
						<div>
		...
		<div class="flex items-center justify-between gap-3 pt-2">
			{#if initialValue?.id && onDelete}
				<button
					type="button"
					class="rounded-full border border-gray-300 dark:border-gray-800 px-4 py-2 text-sm font-medium text-rose-600 dark:text-rose-300 transition hover:border-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 hover:text-rose-700 dark:hover:text-rose-200"
					on:click={() => onDelete?.()}
					disabled={saving}
				>
					Delete view
			...
			<div class="flex items-center gap-3">
				<button
					type="button"
					class="rounded-full border border-transparent px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 transition hover:bg-gray-100 dark:hover:bg-gray-800"
					on:click={close}
					disabled={saving}
				>
					Cancel
				...
				<button
					type="submit"
					class="rounded-full bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:opacity-50 disabled:hover:bg-indigo-600"
