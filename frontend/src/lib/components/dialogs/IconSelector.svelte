...
<div class="relative">
	<button
		bind:this={buttonRef}
		type="button"
		class={`flex w-[50px] items-center justify-center rounded-full border py-1 text-xl transition ${
			isValid
				? "border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 hover:bg-gray-100 dark:hover:bg-gray-700"
	...
	{#if isOpen}
		<div
			...
			class="absolute w-72 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 shadow-xl z-[9999]"
			...
			<div class="space-y-4">
				...
				<div class="space-y-2">
					...
					<input
						...
						type="text"
						class="w-full rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 px-3 py-2.5 text-sm text-gray-900 dark:text-gray-200 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30"
						placeholder="✨"
				...
				<div class="space-y-2">
					...
					<div class="grid grid-cols-6 gap-2">
						{#each VIEW_ICON_SUGGESTIONS as suggestion (suggestion)}
							<button
								type="button"
								class={`flex h-10 w-10 items-center justify-center rounded-lg border text-xl transition ${
									customInput === suggestion
										? "border-indigo-500 bg-indigo-50 dark:bg-gray-500/10 text-indigo-700 dark:text-indigo-200"
										: "border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-200 hover:border-indigo-500/40"
								}`}
				...
				<button
					type="button"
					class="w-full rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700"
					on:click={handleDone}
				>
					Done
				</button>
