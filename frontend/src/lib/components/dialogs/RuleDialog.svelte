			...
			{:else}
				<div class="relative space-y-2">
					...
					<button
						...
						class="w-full rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-950 px-3 py-2 text-sm text-left outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-600/30 flex items-center justify-between"
					>
						{#if selectedView}
							...
							{:else}
								{#each availableViews as view (view.id)}
									<button
										...
										class="w-full px-3 py-2 text-left hover:bg-gray-700 transition-colors flex items-center gap-2 {selectedViewId ===
										view.id
											? 'bg-gray-700'
											: ''}"
									>
