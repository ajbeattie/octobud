			...
			{:else}
				<div class="flex flex-wrap gap-1.5">
					{#each notification.tags ?? [] as tag (tag.id)}
						<button
							type="button"
							class="group flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium transition hover:opacity-80"
		...
		<div class="max-h-[240px] overflow-y-auto p-2">
			...
			{:else}
				{#each unassignedTags as tag, index (tag.id)}
					...
					<button
						type="button"
						class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition {isHighlighted
