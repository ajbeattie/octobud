<script lang="ts">
	...
	let draggedOverRuleId: string | null = null;
	let localRuleOrder: Rule[] = [];
	let originalRuleOrder: Rule[] = [];
	...
	// Tokenize the query for syntax highlighting
	function tokenizeForSyntax(input: string): SyntaxToken[] {
	...
	function toggleReorderMode() {
		if (reorderMode) {
			...
			reorderMode = true;
			originalRuleOrder = [...sortedRules];
			localRuleOrder = [...sortedRules];
		}
	}
	...
	async function handleSaveReorder() {
		reordering = true;
		try {
			const ruleIds = localRuleOrder.map((r) => r.id);
			...
			localRuleOrder = [...originalRuleOrder];
		} finally {
			reordering = false;
		}
	}
	...
	function handleDragOver(event: DragEvent, ruleId: string) {
		if (!reorderMode || !draggedRuleId) return;
		event.preventDefault();
		draggedOverRuleId = ruleId;
	}
	...
	function handleDrop(event: DragEvent, targetRuleId: string) {
		...
		const draggedIndex = localRuleOrder.findIndex((r) => r.id === draggedRuleId);
		const targetIndex = localRuleOrder.findIndex((r) => r.id === targetRuleId);

		if (draggedIndex !== -1 && targetIndex !== -1 && draggedIndex !== targetIndex) {
			const newOrder = [...localRuleOrder];
			const [draggedRule] = newOrder.splice(draggedIndex, 1);
			newOrder.splice(targetIndex, 0, draggedRule);
			localRuleOrder = newOrder;
		}

		draggedRuleId = null;
		draggedOverRuleId = null;
	}
...
<div>
	<div class="flex items-center justify-between mb-6">
		...
		<button
			...
			class="rounded-full bg-indigo-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700"
		>
			New Rule
		</button>
	</div>
	...
	{#if sortedRules.length === 0}
		...
		>
			...
			<button
				on:click={handleNewRule}
				class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium rounded-full transition-colors"
			>
				Create Rule
			</button>
		</div>
	{:else}
		<div class="space-y-2">
			{#each sortedRules as rule (rule.id)}
				...
				<div
					class="bg-gray-50 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-lg transition-colors {reorderMode
						? 'cursor-move hover:border-gray-300 dark:hover:border-gray-700'
						: ''} {draggedOverRuleId === rule.id ? 'border-blue-600' : ''}"
					draggable={reorderMode}
					role={reorderMode ? "button" : undefined}
					aria-label={reorderMode ? `Reorder rule ${rule.name}` : undefined}
					on:dragstart={(e) => handleDragStart(e, rule.id)}
					on:dragover={(e) => handleDragOver(e, rule.id)}
					on:drop={(e) => handleDrop(e, rule.id)}
				>
					<!-- Collapsed header: All in one row -->
					<button
						type="button"
						class="flex items-center justify-between gap-3 px-3 py-3 w-full text-left {reorderMode
							? 'cursor-move'
						...
						<div class="border-t border-gray-200 dark:border-gray-800 pt-5 px-4 pb-2 space-y-3">
							...
							<div class="flex items-center justify-end gap-2 pt-1">
								<button
									...
									class="px-3 py-1.5 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors"
								>
									Edit
								</button>
								<button
									...
									class="px-3 py-1.5 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-950/30 rounded-lg transition-colors"
								>
									Delete
								</button>
							</div>
			...
			<div class="flex gap-2">
				{#if reorderMode}
					<button
						...
						disabled={reordering}
						class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors disabled:opacity-50"
					>
						Cancel
					</button>
					<button
				...
				{:else}
					<button
						on:click={toggleReorderMode}
						class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded transition-colors"
					>
						Reorder
					</button>
				{/if}
