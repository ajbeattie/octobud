...
export async function login(username: string, password: string): Promise<void> {
	// Use the login API function which handles network/proxy error detection
	...
	// Fetch user info including sync settings after login
	try {
		await fetchUserInfo();
	} catch (err) {
		// Don't fail login if fetching user info fails
		console.error("Failed to fetch user info after login:", err);
	}
}

// Fetch user info including sync settings
...
export async function refreshToken(): Promise<void> {
	...
	refreshPromise = (async () => {
		try {
			...
			// Fetch user info including sync settings after refresh
			try {
				await fetchUserInfo();
			} catch (err) {
				// Don't fail refresh if fetching user info fails
