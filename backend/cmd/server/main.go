...
import (
	...
	apiauth "github.com/ajbeattie/octobud/backend/internal/api/auth"
...
func main() {
	...
	if tokenConfigured {
		apiHandler = api.NewHandler(queries,
			api.WithSyncService(dbConn, githubClient),
	...
	rateLimiter := apiauth.NewRateLimiter(5, 1*time.Minute, logger)

	// Initialize token revocation
	tokenRevocation := apiauth.NewTokenRevocation(logger)
	...
	router.Route("/api", func(r chi.Router) {
		// Public auth routes (login) - with rate limiting
		r.Group(func(r chi.Router) {
			r.Use(apiauth.RateLimitMiddleware(rateLimiter, logger))
			r.Post("/auth/login", authHandler.HandleLogin)
		})

		// Protected auth routes
		r.Group(func(r chi.Router) {
			r.Use(apiauth.JWTMiddleware(cfg.JWTSecret, logger, tokenRevocation))
			r.Use(apiauth.CSRFMiddleware(logger))
			r.Get("/auth/me", authHandler.HandleGetCurrentUser)
			r.Post("/auth/refresh", authHandler.HandleRefreshToken)
			r.Post("/auth/logout", authHandler.HandleLogout)
			r.Put("/auth/credentials", authHandler.HandleUpdateCredentials)
		})
