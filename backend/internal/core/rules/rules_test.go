...
func TestService_GetRule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setupMock(mockQuerier, tt.ruleID)
			service := NewService(mockQuerier)

			ctx := context.Background()
			result, err := service.GetRule(ctx, tt.ruleID)
...
func TestService_ListRules(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			result, err := service.ListRules(ctx)
...
func TestService_GetRulesByViewID(t *testing.T) {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, id int64) {
				expectedRules := []db.Rule{
					{ID: 1, Name: "Rule 1", ViewID: sql.NullInt64{Int64: id, Valid: true}},
					{ID: 2, Name: "Rule 2", ViewID: sql.NullInt64{Int64: id, Valid: true}},
				}
				m.EXPECT().
					GetRulesByViewID(gomock.Any(), sql.NullInt64{Int64: id, Valid: true}).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			result, err := service.GetRulesByViewID(ctx, tt.viewID)
...
func TestService_CreateRule(t *testing.T) {
	tests := []struct {
		...
		setupMock   func(*mocks.MockStore, models.CreateRuleParams)
		expectErr   bool
		checkErr    func(*testing.T, error)
		checkResult func(*testing.T, models.Rule)
	}{
		...
		},
		{
			...
			setupMock: func(m *mocks.MockStore, params models.CreateRuleParams) {
				m.EXPECT().
					GetView(gomock.Any(), int64(1)).
					Return(db.View{ID: 1, Name: "My View"}, nil)
				m.EXPECT().
					ListRules(gomock.Any()).
					Return([]db.Rule{}, nil)
				actionsJSON, err := json.Marshal(params.Actions)
				...
				expectedRule := db.Rule{
					ID:           1,
					Name:         params.Name,
					ViewID:       sql.NullInt64{Int64: 1, Valid: true},
					Actions:      actionsJSON,
					Enabled:      true,
					DisplayOrder: 100,
					CreatedAt:    time.Now(),
		...
		},
		{
			...
			expectErr: true,
			checkErr: func(t *testing.T, err error) {
				require.Contains(t, err.Error(), "invalid viewId")
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockQuerier := mocks.NewMockStore(ctrl)
			tt.setupMock(mockQuerier, tt.params)
			service := NewService(mockQuerier)

			ctx := context.Background()
			result, err := service.CreateRule(ctx, tt.params)
...
func TestService_UpdateRule(t *testing.T) {
	tests := []struct {
		...
		setupMock   func(*mocks.MockStore, int64, models.UpdateRuleParams)
		expectErr   bool
		checkErr    func(*testing.T, error)
		checkResult func(*testing.T, models.Rule)
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, id int64, _ models.UpdateRuleParams) {
				actionsJSON, err := json.Marshal(models.RuleActions{SkipInbox: true})
				if err != nil {
					panic("failed to marshal RuleActions in test setup: " + err.Error())
				}
		...
		},
		{
			...
			setupMock: func(_ *mocks.MockStore, _ int64, _ models.UpdateRuleParams) {
				// No mock expectations - should fail before DB call
			},
			expectErr: true,
			checkErr: func(t *testing.T, err error) {
		...
		},
		{
			...
			setupMock: func(m *mocks.MockStore, _ int64, _ models.UpdateRuleParams) {
				m.EXPECT().
					UpdateRule(gomock.Any(), gomock.Any()).
					Return(db.Rule{}, sql.ErrNoRows)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockQuerier := mocks.NewMockStore(ctrl)
			tt.setupMock(mockQuerier, tt.ruleID, tt.params)
			service := NewService(mockQuerier)

			ctx := context.Background()
			result, err := service.UpdateRule(ctx, tt.ruleID, tt.params)
...
func TestService_DeleteRule(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			err := service.DeleteRule(ctx, tt.ruleID)
...
func TestService_ReorderRules(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ruleIDs: []int64{3, 1, 2},
			setupMock: func(m *mocks.MockStore, ids []int64) {
				for range ids {
					m.EXPECT().
						UpdateRuleOrder(gomock.Any(), gomock.Any()).
						DoAndReturn(func(_ context.Context, arg db.UpdateRuleOrderParams) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockQuerier := mocks.NewMockStore(ctrl)
			tt.setupMock(mockQuerier, tt.ruleIDs)
			service := NewService(mockQuerier)

			ctx := context.Background()
			result, err := service.ReorderRules(ctx, tt.ruleIDs)
