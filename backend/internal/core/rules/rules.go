...
func (s *Service) GetRulesByViewID(ctx context.Context, viewID int64) ([]db.Rule, error) {
	rules, err := s.queries.GetRulesByViewID(ctx, sql.NullInt64{Int64: viewID, Valid: true})
	if err != nil {
...
func (s *Service) ListRules(ctx context.Context) ([]models.Rule, error) {
	rules, err := s.queries.ListRules(ctx)
	if err != nil {
...
func (s *Service) GetRule(ctx context.Context, ruleID int64) (models.Rule, error) {
	rule, err := s.queries.GetRule(ctx, ruleID)
	if err != nil {
...
func (s *Service) CreateRule(
	...
	if hasViewID {
		...
		_, err = s.queries.GetView(ctx, viewIDInt)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
	...
	rule, err := s.queries.CreateRule(ctx, dbParams)
	if err != nil {
...
func (s *Service) UpdateRule(
	...
	if params.Actions != nil {
		...
		dbParams.Actions = pqtype.NullRawMessage{RawMessage: actionsJSON, Valid: true}
	}
	if params.Enabled != nil {
		dbParams.Enabled = sql.NullBool{Bool: *params.Enabled, Valid: true}
	}

	rule, err := s.queries.UpdateRule(ctx, dbParams)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return models.Rule{}, errors.Join(ErrRuleNotFound, err)
		}
...
func (s *Service) DeleteRule(ctx context.Context, ruleID int64) error {
	err := s.queries.DeleteRule(ctx, ruleID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (s *Service) ReorderRules(ctx context.Context, ruleIDs []int64) ([]models.Rule, error) {
	if len(ruleIDs) == 0 {
	...
	for i, ruleID := range ruleIDs {
		...
		err := s.queries.UpdateRuleOrder(ctx, db.UpdateRuleOrderParams{
			ID:           ruleID,
			DisplayOrder: displayOrder,
		})
		if err != nil {
