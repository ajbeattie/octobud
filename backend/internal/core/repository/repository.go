...
func (s *Service) ListRepositories(ctx context.Context) ([]models.Repository, error) {
	repositories, err := s.queries.ListRepositories(ctx)
	if err != nil {
...
func (s *Service) ListRepositoriesAsMap(ctx context.Context) (map[int64]db.Repository, error) {
	repositories, err := s.queries.ListRepositories(ctx)
	if err != nil {
...
func (s *Service) GetRepositoryByID(ctx context.Context, id int64) (db.Repository, error) {
	return s.queries.GetRepositoryByID(ctx, id)
}
...
func (s *Service) UpsertRepository(
	...
	repo, err := s.queries.UpsertRepository(ctx, params)
	if err != nil {
