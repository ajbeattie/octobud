...
func (s *Service) GetByGithubID(ctx context.Context, githubID string) (db.Notification, error) {
	return s.queries.GetNotificationByGithubID(ctx, githubID)
}
...
func (s *Service) ListNotifications(
	...
	repoMap, err := s.IndexRepositories(ctx)
	if err != nil {
	...
	for _, notification := range result.Notifications {
		item, err := s.BuildResponse(ctx, notification, repoMap, evaluator)
		if err != nil {
...
func (s *Service) GetTagsForNotification(
	...
	tags, err := s.queries.ListTagsForEntity(ctx, db.ListTagsForEntityParams{
...
func (s *Service) UpsertNotification(
	...
	notification, err := s.queries.UpsertNotification(ctx, params)
	if err != nil {
...
func (s *Service) UpdateNotificationSubject(
	...
	if err := s.queries.UpdateNotificationSubject(ctx, params); err != nil {
...
func (s *Service) GetNotificationWithDetails(
	ctx context.Context, githubID, queryStr string,
) (models.Notification, error) {
	notification, err := s.GetByGithubID(ctx, githubID)
	if err != nil {
	...
	return s.BuildResponse(ctx, notification, nil, evaluator)
}
