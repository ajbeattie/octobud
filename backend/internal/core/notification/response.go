...
func (s *Service) BuildResponse(
	...
	if notification.RepositoryID != 0 {
		...
		if repoMap != nil {
			...
			repoValue, err = s.queries.GetRepositoryByID(ctx, notification.RepositoryID)
			if err != nil {
...
func (s *Service) IndexRepositories(ctx context.Context) (map[int64]db.Repository, error) {
