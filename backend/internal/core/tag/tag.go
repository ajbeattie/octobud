...
func (s *Service) ListTagsWithUnreadCounts(ctx context.Context) ([]models.Tag, error) {
	...
	for _, tag := range tags {
		...
		unreadCount, err := s.calculateTagUnreadCount(ctx, tag.Slug)
...
func (s *Service) CreateTag(
	...
	tag, err := s.queries.UpsertTag(ctx, db.UpsertTagParams{
		Name:        name,
		Slug:        slug,
		Color:       colorNull,
...
func (s *Service) DeleteTag(ctx context.Context, tagID int64) error {
	err := s.queries.DeleteTag(ctx, tagID)
	if err != nil {
...
func (s *Service) ReorderTags(ctx context.Context, tagIDs []int64) ([]db.Tag, error) {
	if len(tagIDs) == 0 {
	...
	for i, tagID := range tagIDs {
		err := s.queries.UpdateTagDisplayOrder(ctx, db.UpdateTagDisplayOrderParams{
			ID: tagID,
...
func (s *Service) GetTag(ctx context.Context, tagID int64) (db.Tag, error) {
...
func (s *Service) calculateTagUnreadCount(ctx context.Context, tagSlug string) (int64, error) {
