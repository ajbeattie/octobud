...
func (s *Service) GetView(ctx context.Context, viewID int64) (db.View, error) {
	view, err := s.queries.GetView(ctx, viewID)
	if err != nil {
...
func (s *Service) ListViewsWithCounts(ctx context.Context) ([]models.View, error) {
	views, err := s.queries.ListViews(ctx)
	if err != nil {
	...
	for _, view := range views {
		...
		unreadCount, countErr := s.calculateViewUnreadCount(ctx, view.Query)
		if countErr != nil {
	...
	inboxCount, err := s.calculateInboxUnreadCount(ctx)
	if err != nil {
	...
	everythingCount, err := s.calculateEverythingUnreadCount(ctx)
	if err != nil {
	...
	archiveCount, err := s.calculateArchiveUnreadCount(ctx)
	if err != nil {
	...
	snoozedCount, err := s.calculateSnoozedUnreadCount(ctx)
	if err != nil {
	...
	starredCount, err := s.calculateStarredUnreadCount(ctx)
	if err != nil {
...
func (s *Service) CreateView(
	...
	view, err := s.queries.CreateView(ctx, params)
	if err != nil {
	...
	unreadCount, err := s.calculateViewUnreadCount(ctx, view.Query)
	if err != nil {
...
func (s *Service) UpdateView(
	...
	view, err := s.queries.UpdateView(ctx, params)
	if err != nil {
...
func (s *Service) DeleteView(
	...
	linkedRules, err := s.queries.GetRulesByViewID(ctx, sql.NullInt64{Int64: viewID, Valid: true})
	if err != nil {
	...
	if _, err := s.queries.DeleteView(ctx, viewID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func (s *Service) ReorderViews(ctx context.Context, viewIDs []int64) ([]models.View, error) {
	if len(viewIDs) == 0 {
	...
	for _, id := range viewIDs {
		view, err := s.queries.GetView(ctx, id)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
	...
	for i, viewID := range viewIDs {
		...
		err := s.queries.UpdateViewOrder(ctx, db.UpdateViewOrderParams{
			ID:           viewID,
