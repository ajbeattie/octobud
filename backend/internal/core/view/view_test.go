...
func TestService_GetView(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := service.GetView(ctx, tt.viewID)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, name string, query string) {
				...
				m.EXPECT().
					CreateView(gomock.Any(), gomock.Any()).
					Return(expectedView, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, id int64) {
				...
				m.EXPECT().
					UpdateView(gomock.Any(), gomock.Any()).
					Return(expectedView, nil)
				// calculateViewUnreadCount may be called
				m.EXPECT().
					ListNotificationsFromQuery(gomock.Any(), gomock.Any()).
...
func TestService_DeleteView(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, id int64) {
				m.EXPECT().
					GetRulesByViewID(gomock.Any(), gomock.Any()).
					Return([]db.Rule{}, nil)
				m.EXPECT().
					DeleteView(gomock.Any(), id).
					Return(id, nil)
			},
			expectErr:               false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			linkedRuleCount, err := service.DeleteView(ctx, tt.viewID, tt.force)
...
func TestService_ReorderViews(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockStore, ids []int64) {
				...
				for range ids {
					m.EXPECT().
						UpdateViewOrder(gomock.Any(), gomock.Any()).
						DoAndReturn(func(_ context.Context, arg db.UpdateViewOrderParams) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			result, err := service.ReorderViews(ctx, tt.viewIDs)
