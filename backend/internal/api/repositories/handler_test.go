...
import (
	...
	"github.com/ajbeattie/octobud/backend/internal/core/repository/mocks"
	"github.com/ajbeattie/octobud/backend/internal/models"
)

func setupTestHandler(ctrl *gomock.Controller) (*Handler, *mocks.MockRepositoryService) {
	logger := zap.NewNop()
	mockSvc := mocks.NewMockRepositoryService(ctrl)
	return New(logger, mockSvc), mockSvc
}
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockRepositoryService) {
				...
				m.EXPECT().ListRepositories(gomock.Any()).Return(repos, nil)
			},
		...
		},
		{
			...
			setupMock: func(m *mocks.MockRepositoryService) {
				m.EXPECT().ListRepositories(gomock.Any()).Return([]models.Repository{}, nil)
			},
			expectedStatus: http.StatusOK,
			expectedBody: func(t *testing.T, w *httptest.ResponseRecorder) {
				var response listRepositoriesResponse
		...
		},
		{
			...
			setupMock: func(m *mocks.MockRepositoryService) {
				m.EXPECT().ListRepositories(gomock.Any()).Return(nil, errors.New("database error"))
			},
