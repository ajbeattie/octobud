...
func New(logger *zap.Logger, tagSvc tag.TagService) *Handler {
	return &Handler{
		logger: logger,
		tagSvc: tagSvc,
	}
...
func (h *Handler) handleListAllTags(w http.ResponseWriter, r *http.Request) {
	...
	tags, err := h.tagSvc.ListTagsWithUnreadCounts(ctx)
	if err != nil {
		h.logger.Error(
...
func (h *Handler) handleCreateTag(w http.ResponseWriter, r *http.Request) {
	...
	tag, err := h.tagSvc.CreateTag(ctx, req.Name, req.Color, req.Description)
	if err != nil {
		h.logger.Error(
...
func (h *Handler) handleUpdateTag(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		shared.WriteError(w, http.StatusBadRequest, "invalid tag ID")
		return
	}
	...
	tag, err := h.tagSvc.UpdateTag(ctx, tagID, req.Name, req.Color, req.Description)
	if err != nil {
...
func (h *Handler) handleDeleteTag(w http.ResponseWriter, r *http.Request) {
	...
	err = h.tagSvc.DeleteTag(ctx, tagID)
	if err != nil {
		h.logger.Error(
...
func (h *Handler) handleReorderTags(w http.ResponseWriter, r *http.Request) {
	...
	// Reorder tags using service
	tags, err := h.tagSvc.ReorderTags(ctx, tagIDs)
	if err != nil {
		h.logger.Error(
