...
import (
	...
	tagmocks "github.com/ajbeattie/octobud/backend/internal/core/tag/mocks"
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService) {
				mockSvc.EXPECT().
					BulkUpdate(gomock.Any(), models.BulkOpMarkRead, gomock.Any(), gomock.Any()).
					Return(int64(2), nil)
			},
...
func TestHandler_handleBulkAssignTag(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService, mockTagSvc *tagmocks.MockTagService) {
				mockTagSvc.EXPECT().
					GetTag(gomock.Any(), int64(1)).
					Return(db.Tag{ID: 1, Name: "test"}, nil)
				mockSvc.EXPECT().
					GetByGithubID(gomock.Any(), "id1").
					Return(db.Notification{GithubID: "id1"}, nil)
				mockSvc.EXPECT().
					GetByGithubID(gomock.Any(), "id2").
					Return(db.Notification{GithubID: "id2"}, nil)
				mockSvc.EXPECT().
					BulkAssignTag(gomock.Any(), gomock.Any(), int64(1)).
					Return(2, nil)
			},
			expectedStatus: http.StatusOK,
			expectedBody: func(t *testing.T, w *httptest.ResponseRecorder) {
		...
		},
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService, mockTagSvc *tagmocks.MockTagService) {
				mockTagSvc.EXPECT().
					...
					Return(db.Tag{ID: 1, Name: "test"}, nil)
				mockSvc.EXPECT().
					ListNotificationsFromQueryString(gomock.Any(), "is:unread", int32(999999)).
		...
		},
		{
			...
			requestBody:    bulkTagNotificationsRequest{TagID: 0},
			setupMock:      func(*notificationmocks.MockNotificationService, *tagmocks.MockTagService) {},
			expectedStatus: http.StatusBadRequest,
		},
		{
		...
		},
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService, mockTagSvc *tagmocks.MockTagService) {
				mockTagSvc.EXPECT().
					...
					Return(db.Tag{ID: 1, Name: "test"}, nil)
				mockSvc.EXPECT().
					GetByGithubID(gomock.Any(), "not-found").
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler, mockSvc, mockTagSvc := setupTestHandler(ctrl)
			if tt.setupMock != nil {
	...
	}{
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService) {
				...
				mockSvc.EXPECT().
					...
					Return(db.Notification{GithubID: "id2"}, nil)
				mockSvc.EXPECT().
					BulkRemoveTag(gomock.Any(), gomock.Any(), int64(1)).
					Return(2, nil)
			},
			expectedStatus: http.StatusOK,
			expectedBody: func(t *testing.T, w *httptest.ResponseRecorder) {
