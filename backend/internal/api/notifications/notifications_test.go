...
import (
	...
	repositorymocks "github.com/ajbeattie/octobud/backend/internal/core/repository/mocks"
...
func setupTestHandler(
...
) (*Handler, *notificationmocks.MockNotificationService, *tagmocks.MockTagService) {
	logger := zap.NewNop()
	...
	mockRiverClient := dbmocks.NewMockRiverClient(ctrl)

	handler := &Handler{
		logger:        logger,
		queries:       nil, // Not used in handler methods
	...
	return handler, mockNotificationSvc, mockTagSvc
}
...
func TestHandler_handleGetNotification(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setupMock: func(mockSvc *notificationmocks.MockNotificationService) {
				mockSvc.EXPECT().
					GetNotificationWithDetails(gomock.Any(), "not-found", "").
