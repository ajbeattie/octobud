...
var (
	...
	ErrRiverClientNotAvailable   = errors.New("river client not available")
)
...
// New creates a new rules handler
func New(
	logger *zap.Logger,
	ruleSvc rulescore.RuleService,
	viewSvc view.ViewService,
...
func (h *Handler) handleGetRule(w http.ResponseWriter, r *http.Request) {
	...
	ruleID, err := parseRuleIDParam(r)
	if err != nil {
		shared.WriteError(w, http.StatusBadRequest, err.Error())
		return
	}

	rule, err := h.ruleSvc.GetRule(ctx, ruleID)
	if err != nil {
...
func (h *Handler) handleCreateRule(w http.ResponseWriter, r *http.Request) {
	...
	createdRule, err := h.ruleSvc.CreateRule(ctx, params)
	if err != nil {
	...
	if req.ApplyToExisting && h.riverClient != nil {
		ruleID, err := strconv.ParseInt(createdRule.ID, 10, 64)
		if err != nil {
			h.logger.Warn(
				"failed to parse created rule ID",
				zap.String("rule_id", createdRule.ID),
				zap.Error(err),
			)
		} else {
			h.logger.Info("enqueuing apply_rule job", zap.Int64("rule_id", ruleID))
			_, err := h.riverClient.Insert(ctx, jobs.ApplyRuleArgs{
				RuleID: ruleID,
			}, nil)
			if err != nil {
				//nolint:lll // Long error message with multiple zap fields
				h.logger.Error("failed to queue apply_rule job", zap.Int64("rule_id", ruleID), zap.Error(errors.Join(ErrFailedToQueueApplyRuleJob, err)))
				// Don't fail the entire request - rule was created successfully
			} else {
				h.logger.Info("successfully queued apply_rule job", zap.Int64("rule_id", ruleID))
			}
		}
	} else if req.ApplyToExisting {
		ruleID, err := strconv.ParseInt(createdRule.ID, 10, 64)
		if err != nil {
			h.logger.Warn("failed to parse created rule ID", zap.String("rule_id", createdRule.ID), zap.Error(err))
		} else {
			//nolint:lll // Long warning message with multiple zap fields
			h.logger.Warn("applyToExisting is true but riverClient is nil", zap.Int64("rule_id", ruleID), zap.Error(ErrRiverClientNotAvailable))
		}
	}
...
func (h *Handler) handleUpdateRule(w http.ResponseWriter, r *http.Request) {
	...
	updatedRule, err := h.ruleSvc.UpdateRule(ctx, ruleID, updateParams)
	if err != nil {
		if errors.Is(err, rulescore.ErrRuleNotFound) {
...
func (h *Handler) handleDeleteRule(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.ruleSvc.DeleteRule(ctx, ruleID); err != nil {
		if errors.Is(err, rulescore.ErrRuleNotFound) {
			shared.WriteError(w, http.StatusNotFound, err.Error())
			return
		}
...
func (h *Handler) handleReorderRules(w http.ResponseWriter, r *http.Request) {
	...
	// Parse rule IDs
	ruleIDs := make([]int64, 0, len(req.RuleIDs))
	for _, rawID := range req.RuleIDs {
		id, err := strconv.ParseInt(rawID, 10, 64)
		if err != nil {
			shared.WriteError(w, http.StatusBadRequest, fmt.Sprintf("invalid rule id: %s", rawID))
			return
		}
		ruleIDs = append(ruleIDs, id)
	}

	// Reorder rules using service
	rules, err := h.ruleSvc.ReorderRules(ctx, ruleIDs)
	if err != nil {
		if errors.Is(err, rulescore.ErrRuleNotFound) {
			shared.WriteError(w, http.StatusNotFound, err.Error())
			return
...
func parseRuleIDParam(r *http.Request) (int64, error) {
