...
func (h *Handler) HandleUpdateSyncSettings(w http.ResponseWriter, r *http.Request) {
	...
	// Queue a sync job if setup was just completed (first time) and River client is available
	if req.SetupCompleted && !wasAlreadyCompleted && h.riverClient != nil {
		_, err := h.riverClient.Insert(ctx, jobs.SyncNotificationsArgs{}, nil)
		if err != nil {
			// Log error but don't fail the request - sync settings were saved successfully
			// The periodic sync will eventually run anyway
...
func (h *Handler) HandleSyncOlder(w http.ResponseWriter, r *http.Request) {
	...
	// Check if River client is available
