...
func TestHandler_HandleLogin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := createRequest(http.MethodPost, "/api/user/login", tt.requestBody)
...
func TestHandler_HandleGetCurrentUser(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *mocks.MockAuthService) {
				m.EXPECT().GetUser(gomock.Any()).Return(&models.User{
					Username: "admin",
				}, nil)
			},
			expectedStatus: http.StatusOK,
		...
		},
		{
			...
			setupMock:      nil, // GetUser not called when unauthorized
			expectedStatus: http.StatusUnauthorized,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := createRequest(http.MethodGet, "/api/user/me", nil)
...
func TestHandler_HandleUpdateCredentials(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := createRequest(http.MethodPut, "/api/user/credentials", tt.requestBody)
			req = tt.setupContext(req)
			w := httptest.NewRecorder()
