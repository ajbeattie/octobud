...
func (h *Handler) handleListViews(w http.ResponseWriter, r *http.Request) {
	...
	views, err := h.viewSvc.ListViewsWithCounts(ctx)
	if err != nil {
...
func (h *Handler) handleUpdateView(w http.ResponseWriter, r *http.Request) {
	...
	view, err := h.viewSvc.UpdateView(ctx, viewID, name, description, icon, req.IsDefault, queryStr)
	if err != nil {
		...
		if errors.Is(err, viewcore.ErrViewNameAlreadyExists) {
			h.logger.Debug("view name already exists", zap.Int64("view_id", viewID), zap.Error(err))
			shared.WriteError(w, http.StatusConflict, err.Error())
			return
		}
		if errors.Is(err, viewcore.ErrInvalidQuery) {
		...
		// Log unexpected errors for debugging
		h.logger.Error("failed to update view", zap.Int64("view_id", viewID), zap.Error(err))
...
func (h *Handler) handleDeleteView(w http.ResponseWriter, r *http.Request) {
	...
	linkedRuleCount, err := h.viewSvc.DeleteView(ctx, viewID, force)
	if err != nil {
		if errors.Is(err, viewcore.ErrViewNotFound) {
			shared.WriteError(w, http.StatusNotFound, err.Error())
			return
...
func (h *Handler) handleReorderViews(w http.ResponseWriter, r *http.Request) {
	...
	// Parse view IDs
	viewIDs := make([]int64, 0, len(req.ViewIDs))
	for _, rawID := range req.ViewIDs {
		id, err := strconv.ParseInt(rawID, 10, 64)
		if err != nil {
			shared.WriteError(w, http.StatusBadRequest, fmt.Sprintf("invalid view id: %s", rawID))
			return
		}
		viewIDs = append(viewIDs, id)
	}

	// Reorder views using service
	views, err := h.viewSvc.ReorderViews(ctx, viewIDs)
	if err != nil {
		if errors.Is(err, viewcore.ErrViewNotFound) {
			shared.WriteError(w, http.StatusNotFound, err.Error())
			return
...
func parseViewIDParam(r *http.Request) (int64, error) {
	rawID := chi.URLParam(r, "id")
	if rawID == "" {
		return 0, errors.New("id is required")
	}
	viewID, err := strconv.ParseInt(rawID, 10, 64)
	if err != nil {
		return 0, errors.New("invalid view id")
	}
	if viewID <= 0 {
		return 0, errors.New("invalid view id")
	}
