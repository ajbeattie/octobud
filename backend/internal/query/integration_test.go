...
func TestIntegration_EndToEnd(t *testing.T) {
	tests := []struct {
		...
		// Basic queries
		{
			...
			input:        "repo:cli",
			wantContains: []string{"r.full_name", "ILIKE", "$1"},
			wantJoins:    1,
		},
		{
			name:         "reason filter",
			input:        "reason:review_requested",
			wantContains: []string{"n.reason", "ILIKE"},
			wantJoins:    0,
		},
		{
			name:         "is:unread",
			input:        "is:unread",
			wantContains: []string{"n.is_read = FALSE"},
			wantJoins:    0,
		},
		...
		// Logical operators
		{
			...
			wantContains: []string{"r.full_name", "AND", "n.is_read = FALSE"},
			wantJoins:    1,
		},
		{
			name:         "OR operator",
			input:        "repo:cli OR repo:other",
			wantContains: []string{"r.full_name", "OR", "$1", "$2"},
			wantJoins:    1,
		},
		{
			name:         "NOT operator",
		...
		// Grouping
		{
			...
			wantContains: []string{"r.full_name", "OR", "AND", "n.is_read = FALSE"},
			wantJoins:    1,
		},
		...
		},
		{
			name:         "in:snoozed",
			input:        "in:snoozed",
			wantContains: []string{"n.snoozed_until IS NOT NULL", "NOW()"},
			wantJoins:    0,
		},
		{
			name:         "in:anywhere",
		...
		// Free text
		{
			...
			input:        "urgent fix",
			wantContains: []string{"n.subject_title", "ILIKE", "OR", "r.full_name"},
			wantJoins:    1,
		},
		...
		// Complex real-world queries
		{
			...
			wantContains: []string{"r.full_name", "n.reason", "n.is_read = FALSE", "AND"},
			wantJoins:    1,
		},
		{
...
func TestIntegration_InboxQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:        "repo:cli",
			wantDefaults: true, // Should have muted-only default (n.muted = FALSE)
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.input == "" {
				hasDefaults = contains(allWhere, "n.archived = FALSE") &&
					contains(allWhere, "n.muted = FALSE") &&
					contains(allWhere, "n.filtered = FALSE")
				...
				hasDefaults = contains(allWhere, "n.muted = FALSE") &&
					!contains(allWhere, "n.archived = FALSE") // Should NOT have archived default
			} else {
...
func TestIntegration_FullSQL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:      "simple repo filter",
			input:     "repo:cli",
			wantWhere: []string{"r.full_name ILIKE $1"},
			wantArgs:  []string{"%cli%"},
		...
		},
		{
			...
			input:     "reason:review_requested",
			wantWhere: []string{"n.reason ILIKE $1"},
		...
		},
		{
			name:      "is:unread",
			input:     "is:unread",
			wantWhere: []string{"n.is_read = FALSE"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "is:read",
			input:     "is:read",
			wantWhere: []string{"n.is_read = TRUE"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
		...
		},
		{
			name:      "archived:true",
			input:     "archived:true",
			wantWhere: []string{"n.archived = TRUE"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			...
			wantWhere: []string{"n.archived = FALSE"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
		...
		},
		{
			name:      "is:muted",
			input:     "is:muted",
			wantWhere: []string{"n.muted = TRUE"}, // Should NOT have n.muted = FALSE default
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "muted:false",
			input:     "muted:false",
			wantWhere: []string{"n.muted = FALSE"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "snoozed:true",
			input:     "snoozed:true",
			wantWhere: []string{"(n.snoozed_until IS NOT NULL AND n.snoozed_until > NOW())"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "snoozed:false",
			input:     "snoozed:false",
			wantWhere: []string{"(n.snoozed_until IS NULL OR n.snoozed_until <= NOW())"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "explicit AND",
			input:     "repo:cli AND is:unread",
			wantWhere: []string{"(r.full_name ILIKE $1 AND n.is_read = FALSE)"},
			wantArgs:  []string{"%cli%"},
			wantJoins: []string{"LEFT JOIN repositories r ON r.id = n.repository_id"},
		},
		{
			name:      "explicit OR",
			input:     "repo:cli OR repo:other",
			wantWhere: []string{"(r.full_name ILIKE $1 OR r.full_name ILIKE $2)"},
		...
		},
		{
			name:      "NOT operator",
			input:     "NOT author:bot",
			wantWhere: []string{"NOT (n.author_login ILIKE $1)"},
			wantArgs:  []string{"%bot%"},
			wantJoins: []string{},
		},
		{
			...
			wantWhere: []string{"(n.reason ILIKE $1 OR n.reason ILIKE $2)"},
		...
		},
		{
			...
			wantWhere: []string{
				"(n.archived = FALSE AND (n.snoozed_until IS NULL OR n.snoozed_until <= NOW()) AND n.muted = FALSE AND n.filtered = FALSE)",
			},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
			name:      "in:archive",
			input:     "in:archive",
			wantWhere: []string{"(n.archived = TRUE AND n.muted = FALSE)"},
			wantArgs:  []string{},
			wantJoins: []string{},
		},
		{
		...
		},
		{
			name:      "state field",
			input:     "state:open",
			wantWhere: []string{"n.subject_state = $1"},
		...
		},
		{
			...
			wantWhere: []string{"n.author_login ILIKE $1"},
		...
		},
		{
			...
			wantWhere: []string{"n.subject_type ILIKE $1"},
		...
		},
		{
			...
			input: "(repo:cli OR repo:other) AND is:unread",
			wantWhere: []string{
				"((r.full_name ILIKE $1 OR r.full_name ILIKE $2) AND n.is_read = FALSE)",
			},
			wantArgs:  []string{"%cli%", "%other%"},
			wantJoins: []string{"LEFT JOIN repositories r ON r.id = n.repository_id"},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !hasInOp && tt.input != "" && !tt.wantErr && !explicitlyAsksForMuted {
				...
				expectedWhere = append(expectedWhere, "n.muted = FALSE")
			}
...
func TestIntegration_ComplexSQL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:     "repo:cli reason:review_requested is:unread",
			wantWhere: "((r.full_name ILIKE $1 AND n.reason ILIKE $2) AND n.is_read = FALSE)",
			wantArgs:  []string{"%cli%", "%review_requested%"},
		},
		{
			name:      "multiple repos with bot exclusion",
			input:     "(repo:cli OR repo:other) AND NOT author:bot",
			wantWhere: "((r.full_name ILIKE $1 OR r.full_name ILIKE $2) AND NOT (n.author_login ILIKE $3))",
			wantArgs:  []string{"%cli%", "%other%", "%bot%"},
		},
		{
			name:      "comma OR with multiple fields",
			input:     "repo:cli,other reason:review_requested,mention",
			wantWhere: "((r.full_name ILIKE $1 OR r.full_name ILIKE $2) AND (n.reason ILIKE $3 OR n.reason ILIKE $4))",
	...
	tests := []struct {
	...
	}{
		{
			...
			wantWhere: []string{
				...
				"n.muted = FALSE",
				"n.filtered = FALSE", // Empty query gets inbox defaults including filtered
			},
		},
		{
			name:  "simple query gets muted-only defaults",
			...
			wantWhere: []string{
				"r.full_name ILIKE $1",
				"n.muted = FALSE", // Non-empty query without in: only excludes muted
			},
		},
		{
			name:  "explicit in:anywhere - no defaults",
		...
		},
		{
			...
			wantWhere: []string{
				...
				"((n.archived = FALSE AND (n.snoozed_until IS NULL OR n.snoozed_until <= NOW()) AND n.muted = FALSE AND n.filtered = FALSE) AND r.full_name ILIKE $1)",
			},
		},
		{
			...
			wantWhere: []string{
				...
				"(((n.tag_ids && ARRAY(SELECT id FROM tags WHERE slug ILIKE $1) AND TRUE) AND NOT (n.muted = TRUE)) AND n.is_read = FALSE)",
			},
		},
	}
...
func TestIntegration_QueryContext(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			query:          "is:muted",
			wantNoDefaults: true, // Should NOT get n.muted = FALSE default
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hasArchived := strings.Contains(allWhere, "n.archived = FALSE")
			hasSnoozed := strings.Contains(allWhere, "snoozed_until")
			hasMuted := strings.Contains(allWhere, "n.muted = FALSE")
			hasFiltered := strings.Contains(allWhere, "n.filtered = FALSE")
			...
			if tt.wantInboxDefs {
				...
				if strings.Contains(tt.query, "in:inbox") {
					...
					// in:anywhere should have TRUE and no other defaults
					if !strings.Contains(allWhere, "TRUE") {
						t.Errorf("in:anywhere should have TRUE, got WHERE: %v", query.Where)
					}
				} else if strings.Contains(tt.query, "is:muted") || strings.Contains(tt.query, "muted:true") {
					// Explicitly asking for muted should NOT have n.muted = FALSE default
					hasMutedTrue := strings.Contains(allWhere, "n.muted = TRUE")
					if !hasMutedTrue {
						t.Errorf("is:muted or muted:true should have n.muted = TRUE, got WHERE: %v", query.Where)
					}
					if hasMuted {
						t.Errorf("is:muted or muted:true should NOT have n.muted = FALSE default, got WHERE: %v", query.Where)
					}
				}
			}
		})
...
func TestIntegration_FreeText(t *testing.T) {
	...
	expectedParts := []string{
		"n.subject_title ILIKE",
		"n.subject_type ILIKE",
		"r.full_name ILIKE",
		"n.author_login ILIKE",
		"n.subject_state ILIKE",
		"OR",
	}
...
func TestCoverage_StateMultipleValues(t *testing.T) {
	...
	// Should use exact matching (not ILIKE)
